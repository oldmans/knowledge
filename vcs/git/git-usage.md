...
`feature/travis-ci` <==> `origin/feature/travis-ci`

`RefSpec` 一般不会出现在命令行中，而是由命令自动写在配置文件中，但是可以在命令行中直接使用。

例如：`git remote add`，Git 会获取远端上 `refs/heads/` 下面的所有引用，并将它写入到本地的 `refs/remotes/origin`。
...
以下几种方式是等价的，因为 Git 把它们都扩展成 `refs/remotes/origin/master`
...
以上描述了 拉取 `RefSpec`，推送是也需要 `RefSpec`
...
* ssh://[user@]host.xz[:port]/path/to/repository.git
* git://host.xz[:port]/path/to/repository.git
* http[s]://host.xz[:port]/path/to/repository.git
* ftp[s]://host.xz[:port]/path/to/repository.git

扩展了用户名之后的:

* ssh://[user@]host.xz[:port]/~[user]/path/to/repository.git
* git://host.xz[:port]/~[user]/path/to/repository.git
* [user@]host.xz:/~[user]/path/to/repository.git
...
以下内容指明了 Git 如何确定 `<repository>:<branch>`。
...
配置文件中并没有指明 `RefSpecs`，所以 Git 需要按照某种策略执行。
...
执行过 `fetch` 操作的项目都会存在一个 `FETCH_HEAD` 列表，这个列表保存在，其中每一行对应于远程服务器的一个分支。
...
* 如果指定了远程分支，`FETCH_HEAD` 为 指定的 `远程分支`，在这种情况下，不会在本地创建本地远程分支，因为这个操作是 `git pull origin branch-name` 的第一步，而对应的 `pull` 操作，并不会在本地创建新的 branch.


例如：

```sh
# git fetch origin :local-branch <==> git fetch origin master:local-branch
git fetch origin remote-branch:local-branch
```

* 首先执行上面的 fetch 操作
* 使用远程 `remote-branch` 分支在本地创建 `local-branch`，但不会切换到该分支
* 如果本地不存在 `local-branch` 分支，则会自动创建一个新的 `local-branch` 分支
* 如果本地已存在 `local-branch` 分支，并且是 `fast forward` 模式，则自动合并两个分支，否则会阻止以上操作
...
注意：虽然我们取回了更新，fetch 取回的之后的更新的保存位置保存在 `.git/refs/remotes/origin` 目录下，那么如何恢复这些更新到工作区呢？

同步工作区与 `.git` 仓库

工作区为空，以 fetch 回来的某分支的更新在工作区新建分支，并自动与该远程分支建立追踪关系

```sh
git checkout -b [local-branch] [origin]/[remote-branch]
```

上面命令表示，在 `origin/remote-branch` 的基础上，创建一个新分支，并建立追踪关系。
...
pull 操作，不应该涉及 `合并` 或 `变基` 操作
换个说法: pull 应该总是 快速合并 的。 为了达到这样一个效果， 在 pull 操作之前， 我倾向于使用`变基`， 在本地对代码执行合并操作。
...
git push origin :hotfix/BJVEP933         # 删除远程仓库的 hotfix/BJVEP933 分支
...
git log --no-merges                            # 可以将 merge 的 commits 排除在外.
git log --grep=keywords                        # 根据 commit 信息过滤 log
git log -p:                                    # 把每次提交的diff计算出来，作为一个patch显示给你看.另一种方法是git show [SHA].
